...
function Input({ value, expected, marked, onChange, label }: { value: string; expected: string; marked: boolean; onChange: (value: string) => void; label: string }) {
  const correct = compact(value) === compact(expected);
  return <input aria-label={label} className={marked ? (correct ? "correct" : "wrong") : ""} value={value} onChange={(event) => onChange(event.target.value)} placeholder="Vul in" />;
}
...
function LongDivisionTable({ scenario, rows, answers, marked, onChange }: { scenario: DivisionScenario; rows: LongDivisionRow[]; answers: Record<string, string>; marked: boolean; onChange: (id: string, value: string) => void }) {
  ...
  return <div className="arithmetic-workbook__sheet"><div className="arithmetic-workbook__brief"><span>OEFENBLAD · DEEL / LANGDELING FORMAAT</span><code>{scenario.dividend} ÷ {scenario.divisor}</code><strong>Skryf die kwosiënt bo; werk die deelstuk en res onder mekaar deur.</strong></div><p className="arithmetic-workbook__instruction">Volg die Word-formaat: begin links by die dividend, bou die <b>kwosiëntbisse bo</b>, en skryf elke nuwe deelstuk en aftrekwerklyn <b>onder mekaar</b>. <b>NEE → 0</b>; <b>JA → 1</b> en trek M af.</p><section className="arithmetic-workbook__long-sheet" aria-label={`${scenario.label} langdelingformaat`}><div className="long-sheet__head"><div className="long-sheet__bracket"><b>{pad(scenario.divisor, 4)}</b><i>⟌</i><span>{pad(scenario.dividend, 8)}</span></div><div className="long-sheet__quotient"><span>KWOSIËNT</span>{rows.map((row, index) => <Input key={row.step} label={`${scenario.label}, kwosiëntbis ${index + 1}`} value={answers[idFor("deel", key, index, "quotient")] ?? ""} expected={row.quotientBit} marked={marked} onChange={(value) => onChange(idFor("deel", key, index, "quotient"), value)} />)}</div></div><div className="long-sheet__legend"><b>LILA: bou die heelgetal deel van links na regs.</b><b>GROEN: hou die nuwe res vir die volgende ry.</b></div><ol className="long-sheet__worklines">{rows.map((row, index) => <li key={row.step} className={row.decision === "JA" ? "long-sheet__workline long-sheet__workline--yes" : "long-sheet__workline"}><span className="long-sheet__step">{String(index + 1).padStart(2, "0")}</span><label><small>deelstuk</small><Input label={`${scenario.label}, ${row.step}, huidige deelstuk`} value={answers[idFor("deel", key, index, "bringDown")] ?? ""} expected={row.bringDown} marked={marked} onChange={(value) => onChange(idFor("deel", key, index, "bringDown"), value)} /></label><label><small>pas M?</small><select aria-label={`${scenario.label}, ${row.step}, pas deler in`} className={marked ? (correct(index, "decision", row.decision) ? "correct" : "wrong") : ""} value={answers[idFor("deel", key, index, "decision")] ?? ""} onChange={(event) => onChange(idFor("deel", key, index, "decision"), event.target.value)}><option value="">Kies</option><option value="NEE">NEE → 0</option><option value="JA">JA → 1</option></select></label><span className="long-sheet__subtract">{row.decision === "JA" ? `− ${pad(scenario.divisor, Math.max(4, row.bringDown.length))}` : "behou"}</span><label><small>res</small><Input label={`${scenario.label}, ${row.step}, nuwe res`} value={answers[idFor("deel", key, index, "remainder")] ?? ""} expected={row.remainder} marked={marked} onChange={(value) => onChange(idFor("deel", key, index, "remainder"), value)} /></label>{marked && (!correct(index, "bringDown", row.bringDown) || !correct(index, "decision", row.decision) || !correct(index, "quotient", row.quotientBit) || !correct(index, "remainder", row.remainder)) ? <small className="long-sheet__feedback"><b>{row.operation}</b> — kwosiëntbis: {row.quotientBit}.</small> : null}</li>)}</ol></section>{marked ? <div className="arithmetic-workbook__long-result"><span>HEELGETAL DEEL</span><strong>{pad(quotient, 8)}₂ = {quotient}</strong><span>RES</span><strong>{pad(remainder, 4)}₂ = {remainder}</strong></div> : null}</div>;
}

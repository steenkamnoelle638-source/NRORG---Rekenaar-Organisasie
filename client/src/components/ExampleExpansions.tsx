...
function DivisionExamples() {
  ...
  return <section className="embedded-division-set"><header><span>DELING / 2 VOORBEELDE</span><b>Binêre langdeling in die klasformaat.</b></header><ExampleToggle><div className="embedded-division-grid">{examples.map(([title, divisor, dividend, quotient, remainder, steps, answer]) => <section className="embedded-division" key={title}><b>{title}</b><div className="embedded-division__head"><label>Deler<code>{divisor}</code></label><div><label>Heelgetal deel in binêr<code>{quotient}</code></label><label>Getal wat mee gedeel word<code>{dividend}</code></label></div></div><ol>{steps.map((step, i) => <li key={i}>{step}</li>)}</ol><footer><span>Res deel van antwoord in binêr</span><code>{remainder}</code><b>Antwoord: {answer}</b></footer></section>)}</div></ExampleToggle></section>;
}
